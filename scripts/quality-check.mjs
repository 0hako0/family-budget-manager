...
check(
  ...
  expenseEntry.includes("receiptExtraImageUrls") &&
    expenseEntry.includes("続きの写真") &&
    actions.includes("function parseReceiptExtraImageUrls") &&
    schema.includes("receipt_extra_image_urls")
);
...
check("今月の支出合計にローン返済を含む", budget.includes("const total = variableExpenseTotal + fixedCostTotal + loanTotal;") && home.includes("ローン {yen(monthlyExpense.loanTotal)}"));
...
check(
  "Security Advisor: security definer execute is revoked from public",
  securityHardening.includes("revoke execute on function public.create_household_group(text, text, text, numeric) from public, anon") &&
    securityHardening.includes("revoke execute on function public.join_household_by_invite_code(text, text, numeric) from public, anon") &&
